...
def collect_statistics(network: Network, E: np.ndarray, P: np.ndarray, A: np.ndarray, epoch: int, data: dict):
    ...
    if epoch % statistics_frequency == 0:
        # x_copy = np.copy(network.x)
        # t_copy = np.copy(network.t)
...
network = Network(n_inputs=11, n_transformation=4, n_hidden=9, n_outputs=11, training_data=patterns_array, test_data=analogies_array, candidates=candidates, desired_response_function=target, collect_statistics_function=collect_statistics)
